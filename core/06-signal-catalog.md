...
Sensor data — an instanced propulsion signal bridged from a J1939 PGN:
...
Infrastructure data — the vessel-wide time reference published by the elected Time Master per [`08-redundancy.md §8.2`](./08-redundancy.md):
...
This catalog projection is illustrative — protocol DCs like `Pelorus.TimeSync` are not instanced (there is one Time Master per dual-bus domain), so no `instance-field` overlay is set.
